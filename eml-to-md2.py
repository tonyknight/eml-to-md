...
# Set directory paths
email_directories = ["/Users/knight/Desktop/Imported/Tony @ Entelechy Design.mbox/eml", "/Users/knight/Desktop/Imported/Entelechy @ AOL.mbox/eml", "/Users/knight/Desktop/Imported/ROVI Sent.mbox/eml"]
...
for email_directory in email_directories:
    ...
    with open(os.path.join(email_directory, "Errors.txt"), "w") as error_file:
        for filename in os.listdir(email_directory):
            ...
            try:
                with open(os.path.join(email_directory, filename), "r") as email_file:
                    try:
                        # Try text parsing first
                        msg = message_from_file(email_file)
                    except:
                        # Try binary parsing as a fallback
                        email_file.seek(0)
                        msg = message_from_file(email_file)
                ...
                with open(os.path.join(markdown_directory, markdown_filename), "w") as markdown_file:
